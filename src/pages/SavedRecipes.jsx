...
const SavedRecipes = () => {
  ...
  useEffect(()=>{
    ...
    const fetchSavedRecipe = async()=>{
      ...
      try {
        const response=await axios.get(`https://recipeapi-7ex8.onrender.com/recipes/savedRecipes/${userID}`);
  ...
  const deleteRecipe = async(recipeID)=>{
    try {
      const response=await axios.put("https://recipeapi-7ex8.onrender.com/recipes",{recipeID,userID,type:"delete"},{headers:{authorization:cookies.access_token}});
      setSavedRecipes(response.data.savedRecipes)
  ...
  return (
    <div>
      ...
      <div className='grid grid-cols-5 gap-2 px-2 max-sm:grid-cols-1'>
        {savedRecipes.map((recipe)=>(
          ...
          <div key={recipe._id} className='col-span-1 border relative flex flex-col items-center' >
            ...
            <AiOutlineDelete onClick={()=>deleteRecipe(recipe._id)} className='absolute top-2 right-2 text-rose-700 text-xl cursor-pointer'/>
      ...
      {savedRecipes.length===0 && <div className='text-lg text-center'>There is no saved recipes.</div>}
